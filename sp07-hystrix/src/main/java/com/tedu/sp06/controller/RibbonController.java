...
public class RibbonController {
    ...
    @HystrixCommand(fallbackMethod = "getItemsFB")//指定降级的方法名
    public JsonResult<List<Item>> getItems(@PathVariable String orderId) {
        System.out.println("item-service调用");
...
//        ribbon调用
    ...
    @HystrixCommand(fallbackMethod = "decreaseNumberFB")//指定降级的方法名
    public JsonResult decreaseNumber(@RequestBody List<Item> items) { //发送 post 请求
//        return rt.postForObject("http://localhost:8001/decreaseNumber", items, JsonResult.class);
        return rt.postForObject("http://item-service/decreaseNumber", items, JsonResult.class);
    }

    @GetMapping("/user‐service/{userId}")
    @HystrixCommand(fallbackMethod = "getUserFB")//指定降级的方法名
...
//        return rt.getForObject("http://localhost:8101/{1}", JsonResult.class, userId);
    ...
    @HystrixCommand(fallbackMethod = "addScoreFB")//指定降级的方法名
...
//        return rt.getForObject("http://localhost:8101/{1}/score?score={2}", JsonResult.class, userId, score);
    ...
    @HystrixCommand(fallbackMethod = "getOrderFB")//指定降级的方法名
...
//        return rt.getForObject("http://localhost:8201/{1}", JsonResult.class, orderId);
    ...
    @HystrixCommand(fallbackMethod = "addOrderFB")//指定降级的方法名
