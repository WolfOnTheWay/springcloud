...
import com.sdt.fsc.service.contract.record.ICVRRecordService;
...
@Slf4j
@Service
public class CVRRecordServiceImpl implements ICVRRecordService {
        ...
        try {
            if (deviceLoginInfo != null) {
                ...
                if (lUserID.intValue() == -1) {
                    ...
                    log.info("登录失败,NET_DVR_Login_V30 Failed,错误消息:" + errorMsg);
                    return false;
                } else {
    ...
    public Boolean loginV40(DeviceLoginInfo deviceLoginInfo) {
        try {
            if (deviceLoginInfo != null){
                ...
                HCNetSDK.NET_DVR_DEVICEINFO_V40 m_struDeviceInfo = new HCNetSDK.NET_DVR_DEVICEINFO_V40();//设备信息
                ...
                m_struLoginInfo.bUseAsynLogin = false; //是否异步登录：0- 否，1- 是
                m_struLoginInfo.write();
                lUserID = hCNetSDK.NET_DVR_Login_V40(m_struLoginInfo, m_struDeviceInfo);
//                m_struDeviceInfo.read();
//                m_struDeviceInfoV40 = m_struDeviceInfo;
                if (lUserID.intValue() == -1) {
                    String errorMsg = hCNetSDK.NET_DVR_GetErrorMsg(new NativeLongByReference(new NativeLong(hCNetSDK.NET_DVR_GetLastError())));
    ...
    public BaseResponse<Integer> getRecordStatus(CameraIdDTO cameraIdDTO) {
        try {
            ...
            int iGroupNo = (iChannel - 1)/64;
            HCNetSDK.NET_DVR_CHAN_GROUP_RECORD_STATUS m_struChanGroupRecordStatus = new HCNetSDK.NET_DVR_CHAN_GROUP_RECORD_STATUS();
            IntByReference intByReference = new IntByReference(0);
            Pointer lpChanGroupRecordStatus = m_struChanGroupRecordStatus.getPointer();
            m_struChanGroupRecordStatus.write();
            boolean state = hCNetSDK.NET_DVR_GetDVRConfig(lUserID, hCNetSDK.NET_DVR_GET_RECORD_CHANNEL_INFO,
                    new NativeLong(iGroupNo), lpChanGroupRecordStatus, m_struChanGroupRecordStatus.size(), intByReference);
            m_struChanGroupRecordStatus.read();
            if (!state){
                String errorMsg = hCNetSDK.NET_DVR_GetErrorMsg(new NativeLongByReference(new NativeLong(hCNetSDK.NET_DVR_GetLastError())));
                log.info("获取通道录像状态失败,错误消息:" + errorMsg);
                return new BaseResponse<>(CustomErrorType.DVR_WorkState_ERROR, -1);
            } else {
                log.info("获取通道录像状态成功.");
                Integer iByRecord = (int) m_struChanGroupRecordStatus.struChanStatus[iChannel - 1].byRecord;
                return new BaseResponse<>(CustomErrorType.SUCCESS, iByRecord);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public List<?> getFileNameInfo(FileParamDTO fileParamDTO) {
        try {
            List<HCNetSDK.NET_DVR_FINDDATA_V40> list = new ArrayList<>();
            if (lUserID == null || lUserID.intValue() == -1) {
            ...
            HCNetSDK.NET_DVR_FILECOND_V40 pFindCond = new HCNetSDK.NET_DVR_FILECOND_V40();
            ...
            pFindCond.write();
            NativeLong lFindHandle = hCNetSDK.NET_DVR_FindFile_V40(lUserID, pFindCond);
            HCNetSDK.NET_DVR_FINDDATA_V40 lpFindData = new HCNetSDK.NET_DVR_FINDDATA_V40();
            int nativeLong = hCNetSDK.NET_DVR_FindNextFile_V40(lFindHandle, lpFindData).intValue();
            if (nativeLong == -1) {
                String errorMsg = hCNetSDK.NET_DVR_GetErrorMsg(new NativeLongByReference(new NativeLong(hCNetSDK.NET_DVR_GetLastError())));
                log.info("逐个获取查找到的文件信息失败,错误消息:" + errorMsg);
                return null;
            }
            while (nativeLong == 1000 || nativeLong == 1002) {
                if (nativeLong == 1000) {
                    list.add(lpFindData);
                    lpFindData = new HCNetSDK.NET_DVR_FINDDATA_V40();
                }
                nativeLong = hCNetSDK.NET_DVR_FindNextFile_V40(lFindHandle, lpFindData).intValue();
            }
    ...
    public void addManualRecordInfo(ManualRecordInfoParamDTO manualRecordInfoParamDTO) {
        try {
            ...
            List<HCNetSDK.NET_DVR_FINDDATA_V40> finddataV40s = (List<HCNetSDK.NET_DVR_FINDDATA_V40>) getFileNameInfo(fileParamDTO);
            if (finddataV40s != null && finddataV40s.size() > 0) {
                ...
                Iterator<HCNetSDK.NET_DVR_FINDDATA_V40> it = finddataV40s.iterator();
                ...
                //将查询结果入库
                while (it.hasNext()) {
                    HCNetSDK.NET_DVR_FINDDATA_V40 finddataV40 = it.next();
                    String fileName = new String(finddataV40.sFileName, "UTF-8").trim();
                    if (!StringUtils.isEmpty(fileName) && finddataV40.dwFileSize>0){
                        ...
                        recordInfo.setFileSize(String.valueOf(finddataV40.dwFileSize));
                        recordInfo.setStartTime(parseNETDVRTIME(finddataV40.struStartTime));
                        recordInfo.setStopTime(parseNETDVRTIME(finddataV40.struStopTime));
    ...
    /**
     *
     * @description: 获取当前通道录像的起止时间(待确认CVR是否支持)
    ...
    public JSONObject getRecordTimeSpan(RecordTimeSpanParamDTO recordTimeSpanParamDTO) {
        try {
            ...
            if (lUserID == null || lUserID.intValue() == -1) {
                ...
                boolean flag = loginV30(deviceLoginInfo);
                if(!flag){
                    return null;
                }
            }
            ...
            if (!bRet) {
                String errorMsg = hCNetSDK.NET_DVR_GetErrorMsg(new NativeLongByReference(new NativeLong(hCNetSDK.NET_DVR_GetLastError())));
                log.info("获取通道录像起止时间失败,NET_DVR_InquiryRecordTimeSpan Failed,错误消息:" + errorMsg);
                return null;
            }else {
    ...
    /**
     *
     * @description: 停止录像并保存录像标签(待确认CVR是否支持)
    ...
    public BaseResponse<Boolean> StopRecordWithRecordLabel(StopRecordParamDTO stopRecordParamDTO) {
        try {
            ...
            if (finddataV40s!= null && finddataV40s.size()>0) {
                ...
                for (HCNetSDK.NET_DVR_FINDDATA_V40 finddataV40 : finddataV40s) {
                    ManualRecordInfo manualRecordInfo = new ManualRecordInfo();
                    String fileName = new String(finddataV40.sFileName, "UTF-8").trim();
                    if (!"".equals(fileName) && finddataV40.dwFileSize>0){
                        ...
                        manualRecordInfo.setFileSize(String.valueOf(finddataV40.dwFileSize));
                        manualRecordInfo.setStartTime(parseNETDVRTIME(finddataV40.struStartTime));
                        manualRecordInfo.setStopTime(parseNETDVRTIME(finddataV40.struStopTime));
